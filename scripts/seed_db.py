...
def seed_db(reset: bool = False):
  ...
  for book_info in TARGET_BOOKS:
    ...
    # Chunk and Score (using correct paragraph boundaries)
    chunks = chunk_text(body_text)
    print(f"  -> Generated {len(chunks)} natural paragraph pages")
    ...
    db.commit()
    print(f"  -> Saved {len(pages_to_add)} pages for {title}")

  db.close()
  print("Seeding complete!")

if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="Seed LIBRIS database with classic books.")
  parser.add_argument("--reset", action="store_true", help="Reset (drop) all tables before seeding.")
